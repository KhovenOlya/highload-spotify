...
Таблицу `User` можно шардировать по полю `country`, будем относительно равномерно распределять по шардам. `Tracks` по полю `track_id`. `Artist` по полю `artist_id`. Распределение по шардам будет равномерным.  
...
`PostgreSQL`: 1 ведущий узел и 3 ведомых узла (1 master, 2 slaves). В Cassanr'е все узлы равны. 
